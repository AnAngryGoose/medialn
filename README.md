...
Two Python scripts that build a clean, Jellyfin/arr stack-compatible symlink tree from a disorganized media library. No files are touched or moved. Seeding keeps working.
...
Setting up Radarr/Sonarr/Jellyfin with an existing disorganized library is a chicken-and-egg problem — the arr stack wants a clean library to import, but you need the arr stack to clean the library.

Every other tool I tried hit at least one of these walls:

- Renames or moves the actual files — breaks torrent seeding
- Can't separate movies from TV in a mixed folder — you do it manually across thousands of files
- Requires the library to already be reasonably structured before it can match anything
- Uses hardlinks, which fail on mergerfs with `EXDEV`
